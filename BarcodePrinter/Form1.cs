...
public partial class Form1 : AppWindow
{
    ...
    private void UpdateConnection(){var printer=new PrinterSettingsService().Profiles().FirstOrDefault()?.PrinterName??"Seçilmedi";connection.Text=$"Barcode Pro 2.1 Beta 8 · Yönetici     •     Yazıcı: {printer}     •     Veri: {(clientMode?"Server / SQLite":"SQLite 3 / Bağlı")}";}
    ...
    private void Attempt(Action action){try{action();}catch(Exception ex)when(ex is InvalidOperationException or IOException or UnauthorizedAccessException or ArgumentException or System.Text.Json.JsonException or System.ComponentModel.Win32Exception or Microsoft.Data.Sqlite.SqliteException){JsonStore.Log(ex);MessageBox.Show(this,ex is Microsoft.Data.Sqlite.SqliteException?"SQLite veritabanı işlemi tamamlanamadı. Dosya erişimini ve Server durumunu kontrol edin.":ex.Message,"İşlem tamamlanamadı",MessageBoxButtons.OK,MessageBoxIcon.Warning);}}
