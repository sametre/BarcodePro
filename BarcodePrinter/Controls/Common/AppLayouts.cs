...
public sealed class AppFieldGrid : TableLayoutPanel
{
    ...
    public Label AddField(string caption,Control editor,int rowHeight=40)
    {
        ...
        editor.Margin=new Padding(0,6,0,6);editor.Dock=DockStyle.Fill;
        Controls.Add(label,0,row);Controls.Add(editor,1,row);return label;
    }
}
public sealed class AppSection : Panel
{
    public Panel Body { get; }=new(){Dock=DockStyle.Fill,Padding=new Padding(12)};
    public AppSection(string title)
    {
        Dock=DockStyle.Fill;Margin=new Padding(0,0,12,0);Padding=new Padding(1);
        var heading=new Label{Text=title,Dock=DockStyle.Top,Height=32,Padding=new Padding(12,8,0,0),Font=new Font("Segoe UI",9,FontStyle.Bold)};
