...
public sealed class MySqlConnectionPanel : UserControl
{
    ...
    private readonly Label status=new(){Dock=DockStyle.Bottom,Height=76,Padding=new Padding(10),Text="Bağlantıyı ayarlayın, ardından ürünleri önizleyin.",AutoEllipsis=true};
    ...
    public MySqlConnectionPanel(Inventory inventory,string settingsPath)
    {
        ...
        tls.Items.AddRange(["TLS zorunlu · Hosting uyumlu (önerilen)","TLS · Sertifika ve sunucu adı doğrulamalı","TLS tercih edilir · Eski sunucu","TLS kapalı · Yalnızca güvenilen yerel ağ"]);tls.SelectedIndex=0;
        ...
        host.Text=settings.Host;port.Value=Math.Clamp(settings.Port,1,65535);database.Text=settings.Database;username.Text=settings.Username;table.Text=settings.Table;tls.SelectedIndex=settings.SslMode switch{MySqlSslMode.VerifyFull=>1,MySqlSslMode.Preferred=>2,MySqlSslMode.Disabled=>3,_=>0};
    ...
    {
        mapping.EndEdit();var result=new MySqlSourceSettings{Host=host.Text.Trim(),Database=database.Text.Trim(),Username=username.Text.Trim(),Port=(uint)port.Value,Table=table.Text.Trim(),SslMode=tls.SelectedIndex switch{1=>MySqlSslMode.VerifyFull,2=>MySqlSslMode.Preferred,3=>MySqlSslMode.Disabled,_=>MySqlSslMode.Required}};
