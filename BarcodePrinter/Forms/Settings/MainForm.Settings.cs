...
public partial class Form1 {
    private void Settings()
    {
        ...
        flow.Controls.Add(Label("Veriler bu bilgisayarda saklanır. Her başarılı kayıtta önceki sürüm .bak olarak korunur."));
        var path = new TextBox { Text = dataPath, ReadOnly = true, Width = 740 }; flow.Controls.Add(path); flow.Controls.Add(Label(""));
        ...
        flow.Controls.Add(Label("")); flow.Controls.Add(Label("Stok düzeltme, girilen değeri yeni toplam stok olarak kaydeder.\nİade stoğu artırır; fire ve çıkış stoğu azaltır.\nMinimum stok uyarı eşiğidir. Maksimum stok planlama sınırıdır.\nBu sürüm tek bilgisayar ve tek kullanıcı içindir; ağ üzerinden eşzamanlı çalışma sunmaz."));
        var tabs=new TabControl{Dock=DockStyle.Fill,Padding=new Point(16,7),Font=BarcodePrinter.Themes.AppTypography.Body()};
        var sql=new TabPage("MySQL bağlantısı");sql.Controls.Add(new MySqlConnectionPanel(inventory,Path.Combine(Path.GetDirectoryName(Path.GetFullPath(dataPath))!,"mysql-connection.json")));
        var local=new TabPage("Yerel veri ve yazıcı"){AutoScroll=true,Padding=new Padding(16)};
        ...
        content.Controls.Remove(flow);local.Controls.Add(flow);tabs.TabPages.AddRange([sql,company,local]);content.Controls.Add(tabs);
    }
