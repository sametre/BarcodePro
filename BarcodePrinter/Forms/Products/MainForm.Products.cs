...
public partial class Form1
{
    ...
    {
        var grid=Grid();grid.AutoSizeColumnsMode=DataGridViewAutoSizeColumnsMode.None;
        ...
        void Refresh()
        {
            ...
            {
                Image? thumbnail=null;if(File.Exists(p.ImagePath)){try{using var original=Image.FromFile(p.ImagePath);thumbnail=new Bitmap(original,new Size(40,40));images.Add(thumbnail);}catch(ArgumentException){}catch(IOException){}}
        ...
        grid.Disposed+=(_,_)=>{foreach(var image in images)image.Dispose();menu.Dispose();columns.Dispose();};content.Controls.Add(grid);content.Controls.Add(toolbar);content.Controls.Add(actions);Refresh();
    }
}
